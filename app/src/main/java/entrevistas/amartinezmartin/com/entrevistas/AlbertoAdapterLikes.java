...
public class AlbertoAdapterLikes extends RecyclerView.Adapter<AlbertoAdapterLikes.AlbertoViewHolder> {
    ...
    public AlbertoAdapterLikes(ArrayList<Movie> mMovies) {
        mDatasetMovies = mMovies;
    }

    @Override
    public AlbertoAdapterLikes.AlbertoViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        mContext = parent.getContext();
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.cardview_likes, parent, false);
        return new AlbertoViewHolder(view);
    }

    // Replace the contents of a view (invoked by the layout manager)
    @Override
    public void onBindViewHolder(final AlbertoViewHolder holder, final int position) {
        ...
        holder.mRemove.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(Utils.removeMovie(mContext,String.valueOf(mDatasetMovies.get(holder.getAdapterPosition()).getId_()))) {
                    mDatasetMovies.remove(position);
                    notifyItemRemoved(position);
                    notifyItemRangeChanged(position, getItemCount());
                    Toast.makeText(mContext, mContext.getString(R.string.removed), Toast.LENGTH_LONG).show();
                }
            }
        });
